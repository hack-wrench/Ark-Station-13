...
/obj/item/bodypart/proc/get_limb_icon(dropped, mob/living/carbon/update_on)
	...
	if(!(bodypart_flags & BODYPART_PSEUDOPART) && (!(bodyshape & BODYSHAPE_TAUR))) // taur legs never ever render
		for(var/key in markings) // Cycle through all of our currently selected markings.
			...
			accessory_overlay = mutable_appearance(body_marking.icon, "[body_marking.icon_state]_[render_limb_string][gender_modifier]", -body_marking.relevant_layer) // ARK STATION EDIT - MARKINGS - ADD - позволяет некоторым маркингам быть выше мутантских частей тела (морд например)
