...
/obj/structure/dresser/attack_hand(mob/user, list/modifiers)
	...
	switch(choice)
		...
		if("Underwear Color")
			var/new_underwear_color = tgui_color_picker(dressing_human, "Choose your underwear color", "Underwear Color", dressing_human.underwear_color) // ARK STATION EDIT: TGUI COLOR PICKER
		...
		if("Undershirt Color")
			var/new_undershirt_color = tgui_color_picker(dressing_human, "Choose your undershirt color", "Undershirt Color", dressing_human.undershirt_color) // ARK STATION EDIT: TGUI COLOR PICKER
			if(new_undershirt_color)
				dressing_human.undershirt_color = sanitize_hexcolor(new_undershirt_color)
		if("Socks Color")
			var/new_socks_color = tgui_color_picker(dressing_human, "Choose your socks color", "Socks Color", dressing_human.socks_color) // ARK STATION EDIT: TGUI COLOR PICKER
			if(new_socks_color)
		...
		if("Bra Color")
			var/new_bra_color = tgui_color_picker(dressing_human, "Choose your Bra color", "Bra Color", dressing_human.bra_color) // ARK STATION EDIT: TGUI COLOR PICKER
			if(new_bra_color)
