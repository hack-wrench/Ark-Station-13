...
/obj/structure/mirror/proc/change_race(mob/living/carbon/human/race_changer)
	...
	if(HAS_TRAIT(race_changer, TRAIT_USES_SKINTONES))
		...
		var/new_mutantcolor = tgui_color_picker(race_changer, "Choose your skin color:", "Race change", race_changer.dna.features["mcolor"]) //ARK STATION EDIT: TGUI COLOR PICKER
		if(new_mutantcolor)
...
/obj/structure/mirror/proc/change_eyes(mob/living/carbon/human/user)
	var/new_eye_color = tgui_color_picker(user, "Choose your eye color", "Eye Color", user.eye_color_left) // ARK STATION EDIT: TGUI COLOR PICKER
...
/obj/structure/mirror/magic/change_hair(mob/living/carbon/human/user)
	...
	var/new_hair_color = tgui_color_picker(user, "Choose your hair color", "Hair Color", user.hair_color) // ARK STATION EDIT: TGUI COLOR PICKER

	if(new_hair_color)
		user.set_haircolor(sanitize_hexcolor(new_hair_color))
		user.dna.update_ui_block(DNA_HAIR_COLOR_BLOCK)
	if(user.physique == MALE)
		var/new_face_color = tgui_color_picker(user, "Choose your facial hair color", "Hair Color", user.facial_hair_color) // ARK STATION EDIT: TGUI COLOR PICKER
		if(new_face_color)
