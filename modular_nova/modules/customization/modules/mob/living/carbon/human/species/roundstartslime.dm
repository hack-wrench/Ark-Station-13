...
/datum/action/innate/alter_form/proc/alter_hair(mob/living/carbon/human/alterer)
	...
	switch(target_hair)
		...
		if("Hair Color")
			...
			if(!hair_area)
				return
			var/new_hair_color = tgui_color_picker(alterer, "Select your new hair color", "Hair Color Alterations", alterer.dna.features["mcolor"]) // ARK STATION EDIT: TGUI COLOR PICKER
			if(!new_hair_color)
				return
